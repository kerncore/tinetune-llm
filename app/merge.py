...
model = AutoModelForCausalLM.from_pretrained(args.base_model_path, torch_dtype=torch.float16, device_map="auto", trust_remote_code=True, local_files_only=True)
...
for adapter_path in adapter_dirs[1:]:
    model = AutoModelForCausalLM.from_pretrained("./temp-merged-step", torch_dtype=torch.float16, device_map="auto", trust_remote_code=True, local_files_only=True)
